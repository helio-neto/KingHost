  ...
  <div class="container-fluid">
    ...
    <div class="collapse navbar-collapse" id="myNavbar">
      <ul class="nav navbar-nav">
        ...
        <li class="active"><a href="salaViewSingle.php">Verificar Salas</a></li>
      </ul>
      ...
      <p>
        <div class="container">
          <form class="reservarForm" method="post" id="reservarForm" name="reservarForm" action="">           
                <fieldset>
                    <legend>Verifique a disponibilidade das salas</legend>
                ...
                <div class="collapse" id="collapseExample">
                  <div class="card card-body">
                    <h4 class="card-title">Horários INDISPONÍVEIS</h4>
                    <p></p>
                  </div>
                  <fieldset>
...
<script type="text/javascript">
   ...
   $( "#fim" ).hide();
    ...
    $('#salaID').on('change', function(){
          var htmlData = ""; 
          var month,day,hour;
          var salaId = $(this).find("option:selected").val();
          $.ajax({
            type: 'POST',
            url: 'controller.php',
            data: {id:salaId, op: 'getSala' },
            dataType: "JSON",
          }).done(function(response) {
              console.log(response);
              $.each(response.reunioes_sala, function(key, value) {
                inicio = moment(value.inicio_reserva);
                fim = moment(value.fim_reserva);
                htmlData += '<ul><li><span class="glyphicon glyphicon-calendar"></span><b>'+inicio.format('D')+'</b>    '+
                            '<span class="glyphicon glyphicon-time text-primary"></span><b>'+inicio.format('hh:mm')+' : '+fim.format('hh:mm')+'</b></li>'+
                              '<li><b>Observações</b> : '+value.info+'</li></ul>'; 
              });
              
              $('#messageModal').find('.modal-body p').html(htmlData);
              //$('#messageModal').modal('show');
              $('#collapseExample').find('.card-body p').html(htmlData);
